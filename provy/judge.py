...
Fetches semantic eval criteria from ag_eval_configs (managed in the Provy UI).
...
Requirements:
  - Eval criteria must be configured in Provy Eval Manager first.
...
def _write_incident_if_failed(session_id: str, agent: str, all_results: list[dict]) -> None:
    ...
    get_client().table("ag_incidents").insert({
        ...
        "fix_suggestion": (
            ...
            "Adjust criteria or agent prompt in Provy Eval Manager."
        ),
        "status": "open",
    }).execute()
...
def evaluate_session_outputs(
    ...
    """
    Layer 4: Evaluate agent outputs against criteria configured in Provy Eval Manager.
    ...
    Requires:
        - Eval criteria configured in Provy Eval Manager (Settings > Eval Manager).
    ...
    for agent, criteria in criteria_by_agent.items():
        ...
        if agent_results:
            try:
                ...
                print(f"[provy.judge] DB write failed for {agent}: {exc}")
